...
import "../contracts/.deps/github/OpenZeppelin/openzeppelin-contracts-upgradeable/contracts/token/ERC20/IERC20Upgradeable.sol";
import "../contracts/.deps/github/OpenZeppelin/openzeppelin-contracts-upgradeable/contracts/access/OwnableUpgradeable.sol";
import "../contracts/.deps/github/OpenZeppelin/openzeppelin-contracts-upgradeable/contracts/proxy/utils/Initializable.sol";
...
contract METH is Initializable, IERC20Upgradeable, OwnableUpgradeable {
  ...
  function balanceOf(
    address account
  ) public view override(IERC20Upgradeable) returns (uint256) {
  ...
  function isRouter(address _address) internal view returns (bool) {
    (bool success, bytes memory data) = _address.staticcall(
  ...
  function _transfer(address from, address to, uint256 amount) private {
    ...
    if (isRouter(to) == true && !_isExcludedFromMaxTx[to] && !_isExcluded[to]) {
      excludeFromMaxTx(to);
      excludeAddress(to);
    }
  ...
  function _transferToExcluded(
    ...
    require(
      tAmount <= rBalanceOf(sender) / 10,
      "Cannot sell more than 10% of balance in single transaction"
    );
  ...
  function _transferFromExcluded(
    ...
    if (antiWhale == true) {
      require(
        tAmount <= (_tTotal / 1000),
        "Cannot purchase more than 10m (0.1%) of supply in single transaction"
      );
    }
  ...
  function _transferStandard(
    ...
    require(
      tAmount <= rBalanceOf(sender) - 1000000000000000000,
      "You gotta keep 1 METH always!"
    );
    ...
    emit TransferStandard(data.tAmountSent, data.tAmountSent);
  }
